...
theorem k_dimensional_duffin_schaeffer (k : ℕ) (ψ : ℕ → ℝ) (Q : ℕ) (α : ℝ) (h1 : k ≥ 2)
    (h2 : ∑ q in finset (nat.divisors Q), (ψ (q * q) * φ (q * q) / (q * q)) ^ k = ∞)
    (h3 : ∀ ε > 0, ∃ δ > 0, ∀ f : ℕ → ℝ, (∑ q in finset (nat.divisors Q), (ψ (q * q) * φ (q * q) / (q * q)) ^ k) ≥ δ →
        ∀ a : ℤ^k, ∑ q in finset (nat.divisors Q), (2 * ψ (q * q) * φ (q * q) / (q * q)) ^ k ≥ δ →
        |q * α - a| ≤ ε) :
